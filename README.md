...
Struktur kolomnya adalah:
a. id — Primary key yang unik untuk setiap konser.
b. artist — Nama artis, grup musik, atau festival yang mengadakan konser.
c. venue_id — Foreign key yang terhubung ke tabel venues, menunjukkan lokasi konser.
d. date — Tanggal pelaksanaan konser.
e. price — Harga tiket untuk satu penonton (dalam format decimal).
f. stock — Jumlah tiket yang tersedia untuk konser tersebut.
...
Tabel ini menyimpan informasi mengenai tempat atau lokasi konser. Setiap venue memiliki atribut seperti nama, alamat, kota, dan kapasitas penonton. Data pada tabel ini menjadi referensi utama untuk konser yang diselenggarakan. Tabel ini menjadi parent bagi tabel concerts, di mana satu venue dapat digunakan untuk banyak konser (relasi one-to-many). Struktur kolomnya adalah sebagai berikut:

a. id — Primary key yang bersifat auto increment.
b. name — Nama venue atau stadion tempat konser berlangsung.
c. address — Alamat venue.
d. city — Kota tempat venue berada.
e. capacity — Kapasitas maksimal venue dalam menampung penonton.

3. Tabel "orders"

Tabel ini berfungsi untuk mencatat setiap pemesanan tiket yang dilakukan pengguna. Setiap order terhubung langsung ke konser tertentu dan menyimpan informasi mengenai nama pemesan, jumlah tiket yang dibeli, total harga, serta tanggal pemesanan. Struktur kolomnya adalah:

a. id — Primary key unik untuk setiap pemesanan.
b. name — Nama pelanggan atau pembeli tiket.
c. concert_id — Foreign key yang menghubungkan pesanan dengan konser tertentu.
d. quantity — Jumlah tiket yang dipesan.
e. total_price — Total harga pesanan (harga tiket × jumlah tiket).
f. order_date — Waktu pemesanan tiket, secara default diisi otomatis dengan timestamp saat data dibuat.
